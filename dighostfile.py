...
for inode in range( inode_low, inode_hi + 1 ):
	...
	try:
		ncores_busy = int( 0.5 + float( os.popen( 'ssh -x -o ConnectTimeout=1 ' + nodename + ' uptime' ).readline().split()[ 10 ][ :-1 ] ) )
...
for nodename in sorted( ncores_free, key=lambda key: ncores_free[ key ], reverse=True ):
	...
	#write to hostfile
	hostfile.write( nodename + ' slots=' + str( ncores_grab ) + ' max-slots=' + str( ncores_grab ) + '\n' )
	nodefile.write( str( ncores_grab ) + '/' + nodename + '\n' )
	if done: break
hostfile.close()
nodefile.close()
