#! /usr/bin/env python
# select muts, not (all in native and all in design)
...
"""
author: Indigo King 06/2012 (chrisk1@uw.edu)
...
def select_all_muts( nat_obj, name='' ):
	pdblist = cmd.get_names()
	if name == '': name = 'muts_' + nat_obj
	mut_sel = ''
	for pdb in pdblist:
		#skip native
		if pdb == nat_obj: continue
		#skip alignment selections
		if pdb[ :4 ] == 'aln_': continue
		dz_obj = pdb
	...
	#				nat_seq[ iseq ] != dz_seq[ iseq ] :
		...
		mut_sel += ' ) )'
		if pdb != pdblist[ -1 ]: mut_sel += ' or '
	print mut_sel
	cmd.select( name, mut_sel )

cmd.extend( 'select_all_muts', select_all_muts )
